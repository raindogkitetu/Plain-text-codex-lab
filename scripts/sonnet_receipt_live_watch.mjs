...
const WATCH_MS=3*60*60*1000+45*60*1000;
...
async function getJson(url,ms=15000){const c=new AbortController();const t=setTimeout(()=>c.abort(),ms);try{const r=await fetch(url,{headers:{'user-agent':'raindog-sonnet-live-watch/1.0','cache-control':'no-cache'},signal:c.signal});if(!r.ok)throw new Error(`HTTP ${r.status}`);return await r.json()}finally{clearTimeout(t)}}
async function getText(url,ms=30000){const c=new AbortController();const t=setTimeout(()=>c.abort(),ms);try{const r=await fetch(url,{headers:{'user-agent':'raindog-sonnet-live-watch/1.0','cache-control':'no-cache'},signal:c.signal});if(!r.ok)throw new Error(`HTTP ${r.status}`);return await r.text()}finally{clearTimeout(t)}}
...
while(!state.receipt&&Date.now()<stopAt){
  ...
  }catch(e){state.read_errors.push(`${new Date().toISOString()}:${e?.name||'Error'}:${e?.message||String(e)}`);await new Promise(r=>setTimeout(r,3000))}
...
else state.status='WATCH_WINDOW_ENDED_NO_VERIFIED_RECEIPT';
save(state);
